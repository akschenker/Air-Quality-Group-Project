...
d3.json(url).then(function(data) {
  ...
  // console.log(buttonCities[0]['pollutant']['pm10']);

//   console.log(buttonCities)
  // var aqi = []
  // buttonCities.forEach(function(d) {
  //   // console.log(d.aqi);
...
function getColor(d) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    var markerColor;
      ...
      var circleSize;
        if (aqiCircle <= 10) {
          circleSize = aqiCircle/1.5
        }
        if (aqiCircle <= 30) {
          circleSize = aqiCircle/2
        }
        if (aqiCircle <= 50) {
          circleSize = aqiCircle/3
        }
        else{
          circleSize = aqiCircle/4
        }
      
      new L.CircleMarker([data[i].latitude, data[i].longitude], {
